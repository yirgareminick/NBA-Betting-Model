...
class FeatureEngineer:
    ...
    def create_upcoming_game_features(self, upcoming_games_df) -> pl.DataFrame:
        """Create features for upcoming games (schedule data without results)"""
        print("🔧 Creating upcoming game features...")
        
        # Convert pandas DataFrame to polars if needed
        if hasattr(upcoming_games_df, 'columns') and not hasattr(upcoming_games_df, 'select'):
            import pandas as pd
            if isinstance(upcoming_games_df, pd.DataFrame):
                upcoming_games_df = pl.from_pandas(upcoming_games_df)
        
        features_list = []
        
        # Expected feature columns in exact order from model metadata 
        # Note: Model has duplicate is_home feature, so we need two columns with different names
        expected_features = [
            'is_home',
            'avg_pts_last_10', 
            'avg_pts_allowed_last_10',
            ...
            'season_avg_pts',
            'season_avg_pts_allowed',
            'is_home_duplicate'  # Duplicate feature in model - renamed to avoid conflict
        ]
        
        # Process each game to create team-level features
        for game in upcoming_games_df.iter_rows(named=True):
            # Home team features - using exact feature names and order from model
            home_features = {
                'is_home': 1,
                'avg_pts_last_10': 110.0,
                'avg_pts_allowed_last_10': 108.0,
                'avg_point_diff_last_10': 2.0,  # pts - pts_allowed
                'win_pct_last_10': 0.55,  # Home advantage
                'win_pct_last_5': 0.55,
                'avg_point_diff_last_5': 2.0,
                'rest_days': 1,
                'game_number_in_season': 30,  # Mid-season default
                'season_win_pct': 0.55,
                'season_avg_pts': 110.0,
                'season_avg_pts_allowed': 108.0,
                'is_home_duplicate': 1  # Duplicate feature to match model
            }
            
            # Away team features
            away_features = {
                'is_home': 0,
                'avg_pts_last_10': 108.0,
                'avg_pts_allowed_last_10': 110.0,
                'avg_point_diff_last_10': -2.0,  # pts - pts_allowed
                'win_pct_last_10': 0.45,  # Away disadvantage
                'win_pct_last_5': 0.45,
                'avg_point_diff_last_5': -2.0,
                'rest_days': 1,
                'game_number_in_season': 30,  # Mid-season default
                'season_win_pct': 0.45,
                'season_avg_pts': 108.0,
                'season_avg_pts_allowed': 110.0,
                'is_home_duplicate': 0  # Duplicate feature to match model
            }
            
            features_list.append(home_features)
            features_list.append(away_features)
        
        # Convert to polars DataFrame with correct column order
        features_df = pl.DataFrame(features_list)
        
        # Rename the duplicate column back to is_home to match model exactly
        features_df = features_df.rename({'is_home_duplicate': 'is_home'})
        ...
        model_expected_features = [
            ...
            'season_avg_pts',
            'season_avg_pts_allowed',
            'is_home'  # Duplicate feature in model
        ]
        features_df = features_df.select(model_expected_features)
        
        print(f"✓ Created upcoming game features: {features_df.shape[0]} rows x {features_df.shape[1]} columns")
        return features_df
